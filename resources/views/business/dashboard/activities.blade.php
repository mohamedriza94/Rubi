...
<div class="row page-titles">
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <input type="text" id="searchActivities" class="form-control" placeholder="Search by User name">
                </div>
            </div>
        </div>
    ...
    <script>
        $(document).ready(function(){
            ...
            var url = "{{ url('business/dashboard/readActivities/:limit') }}";
            function configureUrl()
            {
                var length = $('#searchActivities').val().length;
                if (length == 0) {
                    url = "{{ url('business/dashboard/readActivities/:limit') }}";
            ...
            function readActivities()
            {
                    ...
                    success:function(response){
                        ...
                        $.each(response.data,function(key,item){
                            ...
                            $('#activityTable').append('<tr>\
                                <td><img style="border-radius:100px; width:40px; height:40px;" class="m-r-10" src="'+item.userPhoto+'">'+item.user+' (<span class="text-capitalize">'+item.userType+'</span>)</td>\
                                <td>'+item.activity+'</td>\
                                <td>'+date+' '+time+'</td>\
                                </tr>'); 
                            });
                ...
                $(document).on('keyup', '#searchActivities', function(e) {
                    url = "{{ url('business/dashboard/searchActivities/:search/:limit') }}";
